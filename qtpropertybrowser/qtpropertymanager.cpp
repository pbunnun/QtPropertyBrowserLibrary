...
{
    QtFilePathPropertyManager *q_ptr = nullptr;
...
public:
    ...
    typedef QHash<const QtProperty *, Data> PropertyValueMap;
...
{
    QtPathPropertyManager *q_ptr = nullptr;
...
void QtBoolPropertyManager::setTextVisible(QtProperty *property, bool visible)
{
    const auto it = d_ptr->m_values.find(property);
    if (it == d_ptr->m_values.end())
        return;
